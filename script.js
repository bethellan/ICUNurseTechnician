...
function ensureSettingsOverlay() {
    ...
    overlay.innerHTML = `
        <div class="settings-panel">
            ...
            <div class="settings-preferences" aria-label="Communication display settings">
                ...
                <label for="settingsSpeechVoice">Voice for generated speech</label>
                <div class="settings-inline-row settings-voice-row">
                    ...
                    <button type="button" class="management-btn small-management-btn" data-preview-speech-voice>Preview voice</button>
                </div>
                <p class="settings-help">Used only when a phrase does not have a recorded voice. Available voices depend on the device/browser.</p>
                <label for="settingsSpeechRate">Generated speech speed: <span id="settingsSpeechRateValue">Normal</span></label>
                <input id="settingsSpeechRate" class="settings-range" type="range" min="0.7" max="1.2" step="0.05" value="0.9">
                <label for="settingsSpeechPitch">Generated speech pitch: <span id="settingsSpeechPitchValue">Normal</span></label>
    ...
    overlay.addEventListener('change', (event) => {
        ...
        if (event.target && event.target.id === 'settingsSpeechVoice') {
            ...
            saveAppSettings({ render: false });
            showToast(appSettings.speechVoiceName ? 'Generated speech voice saved' : 'Default device voice selected', 'success');
            return;
        }
...
function normaliseAppSettings(rawSettings) {
    ...
    return { displayMode, pressActivation, autoUpdateCheck, popupCloseDelaySeconds, speechVoiceName, speechVoiceLang, speechRate, speechPitch, introduction };
}
